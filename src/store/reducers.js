...
export const exchange = (
  state = { event: [], allOrders: [], fillOrders: [], status: {} },
  action
) => {
  switch (action.type) {
    ...
    case "EXCHANGE_REQUEST_SUCCESS":
      ...
      let name = nameArr.find(
        (item) => item.requestType == action.requestType
      )?.name;
      let obj = {
        ...state,
        status: {
      ...
      return obj;
    case "TOKENSWAP_LOADED":
      return {
        ...state,
        TokenSwap: action.contract,
      };
